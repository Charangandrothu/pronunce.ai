...
import { UploadCloud, AlertCircle, CheckCircle2, Trash2 } from 'lucide-react';
    ...
    <div className="w-full max-w-xl mx-auto px-4 select-none">
      <div className="glass-panel glow-card bg-[#121215]/50 border border-white/5 rounded-3xl p-6 sm:p-8 relative overflow-hidden shadow-2xl">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6 text-center">
          <div className="p-3 rounded-2xl bg-zinc-950/20 border border-white/5">
            <span className="block text-[9px] text-slate-500 uppercase tracking-widest font-bold">Language</span>
            <span className="block text-xs text-slate-350 mt-1 font-semibold">English</span>
          </div>
          <div className="p-3 rounded-2xl bg-zinc-950/20 border border-white/5">
            <span className="block text-[9px] text-slate-500 uppercase tracking-widest font-bold">Duration</span>
            <span className="block text-xs text-slate-355 mt-1 font-semibold">30s - 45s</span>
          </div>
          <div className="p-3 rounded-2xl bg-zinc-950/20 border border-white/5">
            <span className="block text-[9px] text-slate-500 uppercase tracking-widest font-bold">Form</span>
            <span className="block text-xs text-slate-355 mt-1 font-semibold">Single File</span>
          </div>
        </div>
        ...
        <div
          ...
          className={`relative group cursor-pointer border border-dashed rounded-2xl p-8 sm:p-12 transition-all duration-300 ${
            isDragActive 
              ? 'border-slate-400 bg-white/[0.02]' 
              : file 
                ? 'border-slate-800 bg-white/[0.01]' 
                : 'border-white/8 hover:border-slate-700 bg-white/[0.01] hover:bg-white/[0.02]'
          }`}
        >
          <input {...getInputProps()} />
          ...
          <div className="flex flex-col items-center justify-center space-y-4">
            ...
            {file ? (
              <div className="text-center w-full">
                <span className="inline-flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full text-[9px] font-bold tracking-widest uppercase bg-white/5 text-slate-300 border border-white/10 mb-2">
                  {fileExtension} File
                </span>
                ...
                <div className="h-8 flex items-center justify-center space-x-1 mt-4 max-w-xs mx-auto overflow-hidden opacity-60">
                ...
                <button
                  type="button"
                  onClick={clearFile}
                  className="mt-5 inline-flex items-center space-x-1.5 px-3.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-full transition-colors cursor-pointer border border-white/5"
                >
              ...
              <div className="text-center">
                <div className="w-5 h-5 border-2 border-t-transparent border-slate-400 rounded-full animate-spin mx-auto mb-3"></div>
                <p className="text-xs font-semibold text-slate-350">Evaluating audio boundaries...</p>
              </div>
            ) : (
              <div className="text-center">
                <p className="text-sm font-semibold text-slate-300 group-hover:text-white transition-colors duration-300">
                  Drag speech sample here, or <span className="text-slate-200 group-hover:text-white underline decoration-dotted underline-offset-4">browse files</span>
                </p>
        ...
        <AnimatePresence>
          {error && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -6 }}
              className="mt-4 p-3.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 flex items-start space-x-2.5 text-xs shadow-lg"
            >
        ...
        <motion.button
          ...
          disabled={!file}
          className={`w-full mt-6 py-3.5 rounded-xl text-xs font-bold uppercase tracking-widest flex items-center justify-center transition-all duration-300 cursor-pointer ${
            file
              ? 'bg-white hover:bg-slate-100 text-zinc-950 shadow-md hover:shadow-lg'
        ...
        <div className="text-center mt-4.5">
          <button
            ...
            className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 hover:text-white transition-colors underline underline-offset-4 cursor-pointer"
          >
